...
class Micro:
    ...
    def ratelimit(self, status:int, body:str):
        try:
            if status == 429:
                ...
                print(f'[Micro] -> Discord Ratelimit :: {data["retry_after"]}')
    ...
    def display_banner(self):
        return self.replace_all(f'''
         ...
         ██████╦╝░░░██║░░░  ╚█████╔╝██║░░██║██║░░░░░██║░░██║██║░╚███║╚██╔═██╔══╝╚════██║██████╔╝╚█████╔╝╚█████╔╝
         ╚═════╝░░░░╚═╝░░░  ░╚════╝░╚═╝░░╚═╝╚═╝░░░░░╚═╝░░╚═╝╚═╝░░╚══╝░╚═╝░╚═╝░░░░░░░░╚═╝╚═════╝░░╚════╝░░╚════╝░  
           hehe  melly qt                       also thanks alex for colour kiss kiss
                                                                                             ramphantknight nuked toronto server
            {Fore.RED}════════════════════════════════════{Fore.RESET}
        ''', [')', '('], [f'{Fore.RED}){Fore.RESET}', f'{Fore.RED}({Fore.RESET}'])

    def input_loop(self):
        self.clear_console()

        print(self.display_banner() + f'''
            [{Fore.RED}1{Fore.RESET}] token analer (fucker)
            [{Fore.RED}2{Fore.RESET}] token login
            [{Fore.RED}3{Fore.RESET}] token info
...
if __name__ == '__main__':
    token = Micro.ask(f'[{Fore.RED}>{Fore.RESET}] Enter Token')
    threads = Micro.ask(f'[{Fore.RED}>{Fore.RESET}] Enter Threads')

    client = Micro(token, threads)
    if not client.check_token():
        print(f'[{Fore.RED}!{Fore.RESET}] Invalid token input.')
    else:
        client.input_loop()
